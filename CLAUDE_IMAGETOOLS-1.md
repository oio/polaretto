# Claude ImageTools - Svelte/SvelteKit Image Optimization Library
...
let { src } = $props(); // src = "./hero.jpg" (string)
...
│   │   │   └── analyzer.ts             # Component usage analyzer
│   │   ├── plugin/
...
export function svelteImagePlugin(options: ImagePluginOptions = {}): Plugin {
  ...
  return {
    ...
    async load(id) {
      ...
      const result = await transformImage(filepath, {
        formats,
        breakpoints,
        placeholder,
        params,
...
export async function transformImage(
  ...
  for (const format of formats) {
    for (const width of sizes) {
      ...
      // Process image
      const processed = await processImageVariant(image, {
        width,
        height,
        format,
        filepath,
  ...
  const sourcesByFormat = groupByFormat(variants);
...
async function processImageVariant(
  ...
  // In dev mode, create virtual module
  // In build mode, emit asset
  if (config.command === 'build') {
    // TODO: Use this.emitFile() from Rollup plugin context
    // For now, return URL pattern
  }

  return { url, size: buffer.length };
}

function groupByFormat(variants: any[]) {
  ...
  return Object.entries(groups).map(([format, vars]) => ({
    ...
    sizes: generateSizesAttribute(vars),
  }));
}
...
function generateSizesAttribute(variants: any[]): string {
...
export function transformMarkup(
  ...
  walk(ast.html, (node: any) => {
    if (node.type === 'InlineComponent' && componentNames.includes(node.name)) {
      ...
      // Only process string literals (e.g., src="./image.jpg")
      // Skip if src is already an expression (e.g., src={imageData})
      if (srcAttr.value?.length === 1 && srcAttr.value[0].type === 'Text') {
        ...
        const absolutePath = resolve(dirname(filename), imagePath);

        // Make it relative from project root for the import
        const importPath = imagePath.startsWith('.')
          ? imagePath
          : './' + imagePath;

        // Build query parameters based on component type and attributes
        ...
        // Replace src="./image.jpg" with src={__img_0}
        const start = srcAttr.value[0].start;
        const end = srcAttr.value[0].end;

        // Find the actual quote positions
  ...
  if (imports.length > 0) {
    ...
    // Find existing script block or create one
    const scriptTag = ast.instance;

    if (scriptTag) {
      // Insert at the beginning of existing script
      const scriptStart = scriptTag.content.start;
      s.appendLeft(scriptStart, importStatements + '\n');
    } else {
      // Create new script block at the beginning
      const insertPos = content.match(/<script[^>]*>/)?.index ?? 0;
      if (insertPos === 0) {
        s.prepend(`<script>\n${importStatements}\n</script>\n\n`);
      }
    }
...
function buildQueryString(node: any, options: ImagePluginOptions): string {
  ...
  const formatsAttr = attrs.find((a: any) => a.name === 'formats');
  if (formatsAttr && formatsAttr.value?.[0]?.data) {
    params.set('formats', formatsAttr.value[0].data);
  } else if (options.formats) {
    params.set('formats', options.formats.join(','));
  }

  // Look for placeholder type
  const placeholderAttr = attrs.find((a: any) => a.name === 'placeholder');
  if (placeholderAttr && placeholderAttr.value?.[0]?.data) {
    params.set('placeholder', placeholderAttr.value[0].data);
...
**src/lib/preprocessor/analyzer.ts** - Component usage analyzer:

```typescript
/**
 * Analyzes Svelte files to detect image component usage
 * and extract configuration from props
 */

export interface ComponentUsage {
  component: 'Picture' | 'Image' | 'BackgroundImage';
  src: string;
  formats?: string[];
  placeholder?: string;
  loading?: string;
...
export function analyzeComponentUsage(content: string): ComponentUsage[] {
  const usages: ComponentUsage[] = [];

  // Simple regex-based detection
  // In production, use proper AST parsing (already done in markup.ts)
  const regex = /<(Picture|Image|BackgroundImage)\s+[^>]*src=["']([^"']+)["']/g;

  let match;
  while ((match = regex.exec(content)) !== null) {
    usages.push({
      component: match[1] as any,
      src: match[2],
    });
  }
...
<script lang="ts">
  ...
  let sizes = $derived(src?.sources[0]?.sizes || '100vw');
...
**CRITICAL**: Users must configure BOTH the preprocessor and Vite plugin.
...
import adapter from '@sveltejs/adapter-auto';
import { vitePreprocess } from '@sveltejs/vite-plugin-svelte';
import { svelteImagePreprocessor } from 'svelte-imagetools';
...
import { sveltekit } from '@sveltejs/kit/vite';
import { defineConfig } from 'vite';
import { svelteImagePlugin } from 'svelte-imagetools';

export default defineConfig({
  plugins: [
    // Vite plugin handles the actual image transformation
    svelteImagePlugin({
      formats: ['avif', 'webp', 'jpeg'],
      breakpoints: [640, 768, 1024, 1280, 1536],
      placeholder: 'blur',
...
1. **Preprocessor** (runs first): Scans `.svelte` files, finds `<Picture src="./hero.jpg" />`, rewrites to `<Picture src={__img_0} />` and adds `import __img_0 from './hero.jpg?responsive'`
2. **Vite Plugin**: Sees the import, processes `hero.jpg` with Sharp, generates optimized variants, returns `ImageData` object
3. **Component**: Receives `ImageData` object directly, renders perfect HTML immediately (SSR works!)
...
```svelte
<script>
  import { Image } from 'svelte-imagetools';
</script>
...
```svelte
<script>
  import { Picture } from 'svelte-imagetools';
</script>
...
<Picture
  ...
  artDirectives={[
    { media: '(max-width: 768px)', src: '/images/landscape-portrait.jpg' },
    { media: '(min-width: 769px)', src: '/images/landscape-wide.jpg' }
  ]}
/>
```
...
```svelte
<script>
  import { BackgroundImage } from 'svelte-imagetools';
</script>
...
## Migration Guide (Astro → Svelte)

For users migrating from Astro ImageTools:
...
<Picture src="/images/hero.jpg" alt="Hero" />
```
...
Key differences:
- Use `<script>` block for imports (standard Svelte)
- Props use Svelte's reactivity system
...
### Functionality
- ✅ Generate responsive srcset/sizes
- ✅ Support AVIF, WebP, JPEG, PNG
- ✅ Art direction with `<picture>`
- ✅ Lazy loading with placeholders
- ✅ Automatic breakpoint calculation
- ✅ Background image support

### Performance
- ✅ Build time < 100ms per image (cached)
- ✅ Dev mode HMR < 50ms
...
- **Svelte 5 Runes**: docs/bibles/SVELTE_BIBLE.md (sections on $props, $derived, $effect, $state)
- **SvelteKit Architecture**: docs/bibles/SVELTEKIT_BIBLE.md (sections on routing, SSR, build)
- **Vite Plugin API**: docs/bibles/VITE_BIBLE.md (sections on plugin hooks, asset handling)
- **Sharp API**: docs/bibles/SHARP_BIBLE.md (sections on resize, format, metadata)
- **Astro Patterns**: docs/bibles/ASTRO_IMAGETOOLS_BIBLE.md (implementation reference)
- **HTML Standards**: docs/html/responsive_images.md (srcset, sizes, picture element)
...
```svelte
<!-- Component receives string path -->
<Picture src="./hero.jpg" alt="Hero" />

<!-- Component tries to dynamically import -->
<script>
  let { src } = $props(); // src = "./hero.jpg"
  let imageData = $state(null);

  $effect(() => {
    // ❌ Vite can't analyze this!
    // ❌ Path resolves wrong!
    // ❌ SSR doesn't run $effect!
    import(`${src}?responsive`).then(mod => imageData = mod.default);
  });
</script>

{#if imageData}
  <img src={imageData.src} /> <!-- ❌ Not in SSR HTML! -->
{/if}
```

**Problems:**
- Dynamic import breaks Vite bundling
- Path resolution fails
- No SSR - initial HTML has no `<img>` tag
- LCP regression, SEO disaster

### ✅ New Approach (WORKS)

```svelte
<!-- User writes the same simple API -->
<Picture src="./hero.jpg" alt="Hero" />

<!-- Preprocessor rewrites to: -->
<script>
  import __img_0 from "./hero.jpg?responsive"; // ✅ Static import!
</script>
<Picture src={__img_0} alt="Hero" />

<!-- Component receives ImageData object -->
<script>
  let { src } = $props(); // src = ImageData object
  // ✅ No $effect, no dynamic import!
</script>

<picture>
  <source srcset={src.sources[0].srcset} /> <!-- ✅ In SSR HTML! -->
  <img src={src.src} /> <!-- ✅ Perfect SSR! -->
</picture>
```
...
- ✅ Perfect SSR with `<img>` in initial HTML
- ✅ Zero LCP regression, perfect SEO

## Implementation Checklist

### Week 1-2: Foundation
- [ ] Initialize SvelteKit library project
- [ ] Install dependencies (Sharp, magic-string, types)
- [ ] **Create preprocessor with markup transformation**
- [ ] **Implement AST walking and rewriting logic**
- [ ] Create Vite plugin skeleton with resolveId/load hooks
- [ ] Implement basic Sharp integration
- [ ] Add cache system (memory + disk)
- [ ] **Test preprocessor → plugin → component pipeline**

### Week 3-4: Components
- [ ] **Implement Picture component as "dumb renderer" (receives ImageData, not string)**
- [ ] **Remove all $effect and dynamic imports from components**
- [ ] **Ensure components render synchronously for perfect SSR**
- [ ] Implement Image component (same approach)
- [ ] Implement BackgroundImage component (same approach)
- [ ] Update TypeScript types (PictureProps.src is ImageData, not string)
- [ ] Create demo routes for testing
- [ ] **CRITICAL: Verify SSR outputs <img> in initial HTML**

### Week 5-6: Features
- [ ] Add art direction support
- [ ] Implement all placeholder types
- [ ] Add automatic breakpoint calculation
